...
import {
  checkExistingKeyName,
  indexRecordsBulk,
} from "./elasticsearch/elasticSearchService";
    ...
    eachMessage: async ({ topic, partition, message }) => {
      ...
      if (topic === "translation.upsert.topic") {
        ...
        for await (const translation of translations) {
          const results = await checkExistingKeyName(translation.keyName);
          if (results.length === 0) await indexRecordsBulk([translation]);
